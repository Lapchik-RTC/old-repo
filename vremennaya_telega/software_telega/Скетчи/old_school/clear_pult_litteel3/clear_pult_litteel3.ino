...
void loop()
{
  ...
  if ((Y1 < 20) && (Y1 > -20))  Y1 = 0;
  if ((X1 < 20) && (X1 > -20))  X1 = 0;
  if (digitalRead(UP) == 0 && regim == 0) {
    while (digitalRead(UP) == 0) {}
    regim = 1;
  }
  if (digitalRead(UP) == 0 && regim == 1) {
    while (digitalRead(UP) == 0) {}
    regim = 0;
  }
  if (digitalRead(baton3) == 0 && MicroBotn == 0) {
    while (digitalRead(baton3) == 0) {}
    MicroBotn = 1;
  }
  if (digitalRead(baton3) == 0 && MicroBotn == 1) {
    while (digitalRead(baton3) == 0) {}
    MicroBotn = 0;
  }
